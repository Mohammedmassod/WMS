...
$(document).ready(function () {
    ...
    const checklocator = () => {
        ...
        const locator = locatorEl.value;//.trim();

        if (!isRequired(locator)) {
            showError(locatorEl, 'يرجى تعبئة هذا الحقل');
        }
        else {
            showSuccess(locatorEl);
            valid = true;
        }
        return valid;
    };
    ...
    const checkLot = () => {
        ...
        if (!isRequired(lot)) {
            showError(lotEl, 'يرجى تعبئة هذا الحقل');
        }
        else {
            showSuccess(lotEl);
            valid = true;
    ...
    const checkpurposeOfTransport = () => {

        let valid = false;

        const purposeOfTransport = purposeOfTransportEl.value;//.trim();

        if (!isRequired(purposeOfTransport)) {
            showError(purposeOfTransportEl, 'يرجى تعبئة هذا الحقل');
        }
        else {
            showSuccess(purposeOfTransportEl);
            valid = true;
        }
        return valid;
    };
    ...
    button1.addEventListener('click', function (e) {
        ...
        let isQuantityValid = checkQuantity(),
            ...
            ispurposeOfTransportValid = checkpurposeOfTransport(),
            isLotValid = checkLot(),
            isDescriptionItemValid = checkdescriptionItem(),
            isLocatorValid = checklocator();
            //isDateTransportValid = checkdateTransport();



        //submitEl.setAttribute("id", "type-success");
        let isFormValid = isQuantityValid && isSecondaryUnitValid && ispurposeOfTransportValid &&
            isLotValid && isDescriptionItemValid && isLocatorValid;// && isDateTransportValid;
